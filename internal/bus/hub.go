...
func (h *Hub) Serve(conn net.Conn) {
	...
	if !h.handshake(conn, sc, name, oneshot, pub) {
		return
	}
	...
	me, stale, stalePeer := h.register(conn, name, oneshot, out)
	...
	h.announceJoin(conn, name, oneshot, stale, stalePeer)
	if !oneshot {
		h.monitorOnce.Do(func() { go h.monitor() })
	}

	for sc.Scan() {
		h.handleLine(conn, &me, name, sc.Text())
	}
	h.unregister(conn, name, oneshot, out)
}

// handshake runs the identity gate (issue #6). It runs BEFORE
...
func (h *Hub) handshake(conn net.Conn, sc *bufio.Scanner, name string, oneshot bool, pub ed25519.PublicKey) bool {
	if pub == nil {
		...
		h.mu.Unlock()
		if has {
			// Bounded refusal write; the deadline stays armed through
			// every refusal write below: a refused client that stops
			// reading must time out, not park this goroutine forever
			// (PR #20 review).
...
// register adds the connection as a peer and enqueues its welcome.
// A rider's name is its identity: a new join under an existing name
// supersedes the old connection (returned as stale/stalePeer), so
// stale wiring (a dead session's leftover join) cannot cause duplicate
// deliveries. Oneshot senders neither displace nor count. Callers own
// the stale peer's teardown.
func (h *Hub) register(conn net.Conn, name string, oneshot bool, out chan string) (me peer, stale net.Conn, stalePeer peer) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if !oneshot {
	...
	return me, stale, stalePeer
}

// announceJoin tells the displaced connection why it is going away and
// announces the join on the feed. Without the displacement notice its
// operator sees a bare EOF in the rider log, which is
...
func (h *Hub) announceJoin(conn net.Conn, name string, oneshot bool, stale net.Conn, stalePeer peer) {
	if stale != nil {
...
// handleLine processes one line from a registered peer: last-seen and
// responsiveness bookkeeping, then ping/ACK control traffic, then
// delivery of ordinary lines.
func (h *Hub) handleLine(conn net.Conn, me *peer, name, line string) {
	h.mu.Lock()
	*me.seen = time.Now()
	if *me.flagged {
		*me.flagged = false
...
// unregister removes the connection after EOF. "still" is false when
// this conn was displaced by a later same-name join: that join already
// removed it from h.peers (and owns closing its outbox), so
// suppressing the departure notice here avoids reporting a leave that
// would read as the *new* holder leaving.
func (h *Hub) unregister(conn net.Conn, name string, oneshot bool, out chan string) {
	h.mu.Lock()
...
func (h *Hub) deliver(from, text string, via net.Conn) {
	if to, payload, ok := ParseAddressed(text); ok {
		h.deliverAddressed(from, to, payload, via)
		return
	}
...
// deliverAddressed relays a TO-addressed line to exactly one target —
// the peer holding the name, the spool for later, or the local sink —
// so an addressed delivery never wakes an uninvolved agent (ADR 0003).
func (h *Hub) deliverAddressed(from, to, payload string, via net.Conn) {
	...
	default:
		...
		spoolNotice = h.deliverDirectLocked(to, line, via)
	}
	h.mu.Unlock()
...
// addHostEnvelope spools a host-addressed line and returns the
// envelope the local sink should receive (empty when the spool failed,
// with a notice then). The host gets the same durable-first contract
// as remote riders (PR #18 review): spool, deliver enveloped, forget
// only on AckLocal. (Sink call after unlock.) Caller holds h.mu.
func (h *Hub) addHostEnvelope(from, to, line, payload string) (hostEnvelope, spoolNotice string) {
	id, err := h.Spool.Add(to, line)
	if err != nil {
		return "", fmt.Sprintf("could not spool for %s: %v — line lost", to, err)
	}
	return Message(from, Envelope(id, payload)), ""
}

// spoolForRider durably stores an addressed line for a remote target
// and kicks the target's pump when it is connected. Durable first,
// always: every addressed line lands in the spool, the target's pump
// delivers it as an envelope, and the entry survives until the rider's
// ACK (issue #7, ADR 0004). Ordering falls out: the spool is the
...
// delivery, loudly lossy when nobody holds the name.
// Caller holds h.mu.
func (h *Hub) deliverDirectLocked(to, line string, via net.Conn) string {
	delivered := 0
	for conn, p := range h.peers {
		if p.oneshot || p.name != to || conn == via {
			continue
	...
	if delivered == 0 {
		return fmt.Sprintf("nobody holds the name %s — line dropped (no spool)", to)
	}
	return ""
}
